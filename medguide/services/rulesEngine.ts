...
export const executeSafetyRules = (
  ...
  // In a real app, we would calculate end dates. 
  // For this prototype, we check the latest event for each unique medication.
  const latestMedsMap = new Map<string, MedicationEvent>();
  
  medicationEvents.forEach(evt => {
    // Basic deduplication logic based on medication name
    const existing = latestMedsMap.get(evt.medication);
    // Simple logic: if we haven't seen it, add it. If we have, update if date is newer.
    if (!existing || new Date(evt.date) > new Date(existing.date)) {
