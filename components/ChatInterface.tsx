...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ patient, messages, setMessages }) => {
  const [input, setInput] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  
  // Use the cached singleton instance of RAG Engine
  const ragEngine = useRef<ClinicalRAG | null>(null);

  useEffect(() => {
    // Get singleton instance based on patient ID to preserve embeddings
    ragEngine.current = ClinicalRAG.getInstance(patient.notes, patient.id);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { answer, context } = await ragEngine.current.answerQuery(userMsg.content);
