...
import { ExtractionResult, SafetyResult, ViewState, MedicationEvent, AppError, ErrorCategory, ChatMessage } from './types';
...
const App: React.FC = () => {
  ...
  // Default to PATIENT_C (Madam C) as the star demo case
  const [selectedPatientId, setSelectedPatientId] = useState<string>("PATIENT_C");
  ...
  // Helper to get or initialize chat messages for current patient
  const getCurrentChatMessages = (): ChatMessage[] => {
    if (chatCache[selectedPatientId]) return chatCache[selectedPatientId];
    return [{
      id: 'intro',
      role: 'model',
      content: `Hello. I have analyzed ${currentPatient.name}'s records. You can ask me specific questions like "When was the last HbA1c?" or "Why was Warfarin started?"`,
      timestamp: Date.now()
    }];
  };

  const updateCurrentChatMessages = (newMessages: ChatMessage[] | ((prev: ChatMessage[]) => ChatMessage[])) => {
    setChatCache(prev => {
      const currentMessages = prev[selectedPatientId] || getCurrentChatMessages();
      const updatedMessages = typeof newMessages === 'function' ? newMessages(currentMessages) : newMessages;
      return {
        ...prev,
        [selectedPatientId]: updatedMessages
      };
    });
  };
  ...
  // Auto-load Logic: If no data is present on landing, automatically trigger analysis
  // This enables "Instant Demo" feel for pre-computed patients.
  useEffect(() => {
    if (currentView === 'landing' && !extractedData && !isLoading && !error) {
      // Trigger process for the default selected patient (Madam A or whoever is selected)
      handleProcess();
    }
  }, [selectedPatientId]); // Re-run if patient ID changes (though handlePatientChange handles cache, this catches empty state)
